...
void main() {
  group("TemperatureCache", () {
    ...
    test("relative temperature", () {
      final blueTemp = TemperatureCache(Hct.fromInt(0xff0000ff))
          ._inputRelativeTemperature;
      expect(blueTemp, closeToWithPrecision(0.0, 3));

      final redTemp = TemperatureCache(Hct.fromInt(0xffff0000))
          ._inputRelativeTemperature;
      expect(redTemp, closeToWithPrecision(1.0, 3));

      final greenTemp = TemperatureCache(Hct.fromInt(0xff00ff00))
          ._inputRelativeTemperature;
      expect(greenTemp, closeToWithPrecision(0.467, 3));

      final whiteTemp = TemperatureCache(Hct.fromInt(0xffffffff))
          ._inputRelativeTemperature;
      expect(whiteTemp, closeToWithPrecision(0.5, 3));

      final blackTemp = TemperatureCache(Hct.fromInt(0xff000000))
          ._inputRelativeTemperature;
      expect(blackTemp, closeToWithPrecision(0.5, 3));
    });

    test("complement", () {
      final blueComplement = TemperatureCache(Hct.fromInt(0xff0000ff))
          .complement
          .toInt();
      expect(blueComplement, 0xff9d0002);

      final redComplement = TemperatureCache(Hct.fromInt(0xffff0000)).complement
          .toInt();
      expect(redComplement, 0xff007bfc);

      final greenComplement = TemperatureCache(Hct.fromInt(0xff00ff00))
          .complement
          .toInt();
      expect(greenComplement, 0xffffd2c9);

      final whiteComplement = TemperatureCache(Hct.fromInt(0xffffffff))
          .complement
          .toInt();
      expect(whiteComplement, 0xffffffff);

      final blackComplement = TemperatureCache(Hct.fromInt(0xff000000))
          .complement
          .toInt();
