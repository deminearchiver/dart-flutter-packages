...
import 'package:androidx_graphics_shapes/src/shapes/corner_rounding.dart';
import 'package:androidx_graphics_shapes/src/shapes/cubic.dart';
import 'package:androidx_graphics_shapes/src/shapes/feature_mapping.dart';
import 'package:androidx_graphics_shapes/src/shapes/features.dart';
import 'package:androidx_graphics_shapes/src/shapes/point.dart';
import 'package:androidx_graphics_shapes/src/shapes/polygon_measure.dart';
import 'package:androidx_graphics_shapes/src/shapes/rounded_polygon.dart';
  ...
  group('PolygonMeasure', () {
    ...
    void irregularPolygonMeasure(
      ...
      for (var index = 0; index < measuredPolygon.features.length; index++) {
        final ProgressableFeature progressableFeature =
            measuredPolygon.features[index];
        expect(
    ...
    void regularPolygonMeasure(
      ...
      irregularPolygonMeasure(RoundedPolygon(sides, rounding: rounding), (
        measuredPolygon,
      ) {
    ...
    test('measure slightly rounded hexagon', () {
      irregularPolygonMeasure(
        RoundedPolygon(6, rounding: const CornerRounding(radius: 0.15)),
      );
    });

    test('measure medium rounded hexagon', () {
      irregularPolygonMeasure(
        RoundedPolygon(6, rounding: const CornerRounding(radius: 0.5)),
      );
    });

    test('measure maximum rounded hexagon', () {
      irregularPolygonMeasure(
        RoundedPolygon(6, rounding: const CornerRounding(radius: 1)),
      );
    });
    ...
    test('handles empty feature last', () {
      final triangle = RoundedPolygon.fromFeatures([
        Feature.convexCorner([
          CubicBezier.straightLine(Offset.zero, const Offset(1, 1)),
        ]),
        Feature.convexCorner([
          CubicBezier.straightLine(const Offset(1, 1), const Offset(1, 0)),
        ]),
        Feature.convexCorner([
          CubicBezier.straightLine(const Offset(1, 0), Offset.zero),
        ]),
        // Empty feature at the end.
        Feature.convexCorner([
          CubicBezier.straightLine(Offset.zero, Offset.zero),
        ]),
      ]);
