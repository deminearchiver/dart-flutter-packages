...
void main() {
  group('Polygon', () {
    ...
    final roundedSquare = RoundedPolygon(
      4,
    ...
    test('transform', () {
      ...
      final List<CubicBezier> translatedSquareCubics = square
          .transformed(translator)
          .cubics;

      for (var i = 0; i < squareCubics.length; i++) {
        expectPointsEqualish(
          squareCubics[i].anchor0 + offset,
          translatedSquareCubics[i].anchor0,
        );
        expectPointsEqualish(
          squareCubics[i].control0 + offset,
          translatedSquareCubics[i].control0,
        );
        expectPointsEqualish(
          squareCubics[i].control1 + offset,
          translatedSquareCubics[i].control1,
        );
        expectPointsEqualish(
          squareCubics[i].anchor1 + offset,
          translatedSquareCubics[i].anchor1,
        );
      }
    });
    ...
    test('toString names the type and its parts', () {
      ...
      expect(
        description,
        startsWith('RoundedPolygon(center: Offset(0.0, 0.0), features: ['),
      );
      ...
      final RoundedPolygon poly =
          RoundedPolygon(
            ...
            final Point point = Point(
              x,
              y,
            ).rotate(45).scale(648, 648).translate(540, 1212);
    ...
    test('empty', () {
      ...
      final RoundedPolygon stillEmpty = poly.transformed(
        scaleTransform(10, 20),
      );
    ...
    test('empty side', () {
      ...
      // Triangle.
      final poly2 = RoundedPolygon.fromVertices(const [
        Point.zero,
        Point(1, 0),
        Point(0, 1),
      ]);
