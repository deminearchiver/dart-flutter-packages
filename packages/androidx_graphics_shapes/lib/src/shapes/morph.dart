...
class Morph(
  ...
  final _morphMatch = _match(start, end);
  ...
  static List<(CubicBezier, CubicBezier)> _match(
    RoundedPolygon p1,
    RoundedPolygon p2,
  ) {
    // Measure polygons, returns lists of measured cubics for each polygon,
    // which we then use to match start/end curves.
    final measuredPolygon1 = MeasuredPolygon.measure(
      const LengthMeasurer(),
      p1,
    );
    final measuredPolygon2 = MeasuredPolygon.measure(
      const LengthMeasurer(),
      p2,
    );
    ...
    var b1 = bs1.cubicAtOrNull(i1++);
    var b2 = bs2.cubicAtOrNull(i2++);
    ...
    assert(
      b1 == null && b2 == null,
      "Expected both Polygon's CubicBezier to be fully matched",
    );
  ...
  Rect get approximateBounds =>
      start.approximateBounds.expandToInclude(end.approximateBounds);
  ...
  List<CubicBezier> toCubics(double progress) {
    final matchCount = _morphMatch.length;
    ...
    final (firstFrom, firstTo) = _morphMatch[0];
    final firstAnchor0X = lerp(firstFrom.anchor0X, firstTo.anchor0X, progress);
    final firstAnchor0Y = lerp(firstFrom.anchor0Y, firstTo.anchor0Y, progress);

    return List<CubicBezier>.generate(matchCount, (i) {
      final (from, to) = _morphMatch[i];
